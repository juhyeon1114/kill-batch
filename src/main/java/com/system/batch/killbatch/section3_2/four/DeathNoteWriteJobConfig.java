package com.system.batch.killbatch.section3_2.four;
...
public class DeathNoteWriteJobConfig {
	...
	/**
	 * 실제 쓰기 작업을 담당할 "위임 대상 ItemWriter"
	 */
	@Bean
	...
	/**
	 * 읽혀진 데이터를 적절히 "위임 대상 ItemWriter"에게 위임할 "위임자 ItemWriter"
	 */
	@Bean
	@StepScope
	public MultiResourceItemWriter<DeathNote> multiResourceItemWriter(
		...
		return new MultiResourceItemWriterBuilder<DeathNote>()
			...
			.itemCountLimitPerResource(10) // 한 파일당 최대 라인수. (한 파일이 이 라인수를 넘으면 새 파일을 생성한다.)
			.delegate(delegateItemWriter()) // 실제 파일 쓰기 작업을 수행할 "위임 대상 ItemWriter"를 지정.
